...
public class DaoPatternDemo {
	public static void main(String[] args) {
		...
		/*
		 * 1.get student 2.Update student 3.Delete student
		 */
		// print all students
		...
		/*
		 * // comes from Dao by connecting with database Address adr1 = new
		 * Address("mumbai", 123); Address adr2 = new Address("mumbai", 123);
		 * 
		 * List<Address> adrList = new ArrayList<Address>(); adrList.add(adr1);
		 * adrList.add(adr2);
		 * 
		 * List<StudenAddressDTO> sdto = new ArrayList<StudenAddressDTO>();
		 * 
		 * sdto.add(new StudenAddressDTO(studList.get(0).getName(),
		 * adrList.get(0).getCity())); sdto.add(new
		 * StudenAddressDTO(studList.get(1).getName(), adrList.get(1).getCity()));
		 * 
		 * for (StudenAddressDTO dtodata : sdto) { System.out.println(dtodata.getName()
		 * + " " + dtodata.getCity()); }
		 */
	}
}
